...
from monoscene.models.GET_utils import load_checkpoint
...
class Group_Mlp_HW(nn.Module):
    """
    기존 Group_Mlp와 동일한 연산을 수행하되, __init__ 시점에 H, W를 인자로 받아서
    forward 시에 (B, N, C) → (B, H, W, C) 형태로 reshape 후 Convolution을 수행합니다.

    만약 H와 W가 None이면, sqrt(N)를 통해 정사각형으로 가정합니다.
    """
    ...
    def __init__(self, 
        ...
        """
        Args:
            in_features (int): 입력 채널 수(C_in).
            hidden_features (int): 중간 채널 수. 기본값은 in_features.
            out_features (int): 출력 채널 수(C_out). 기본값은 in_features.
            act_layer (nn.Module): 활성화 함수 클래스. 기본 nn.GELU
            drop (float): 드롭아웃 확률.
            norm_layer (nn.Module or None): 사용할 정규화 레이어.
            mlpfp32 (bool): True면 연산을 float32로 강제 변환.
            group_num (int): 그룹 컨볼루션용 그룹 개수.
            height (int): 입력 토큰을 2D로 펼칠 때 사용할 높이(H).
            width (int): 입력 토큰을 2D로 펼칠 때 사용할 너비(W).
        """
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        ...
        #in_features = int(24) #in_features
    ...
    def forward(self, x: torch.Tensor, height: int, width: int) -> torch.Tensor:
        """
        x: (B, N, C) 형태 (또는 이미 4D인 (B, H, W, C) 형태)

        Returns:
            out: (B, N, C') 형태 (또는 4D 그대로라면 (B, H, W, C'))
        """
        B = x.shape[0]
        need_reset_shape = False

        # 만약 x가 (B, N, C) 형식이라면, (B, H, W, C)로 reshape
        if x.dim() != 4:
            need_reset_shape = True
            # H, W가 주어지지 않았다면 sqrt(N)으로 정사각형 추정
            if (height is None) or (width is None):
                N = x.shape[1]
                hw = int(math.sqrt(N))
                if hw * hw != N:
                    # 정사각형이 아니면 부족한 토큰은 패딩 필요할 수도 있음
                    raise ValueError(
                        f"[Group_Mlp_HW] N={N}이 정사각형이 아니므로, "
                        f"height/width를 지정하거나 패딩 로직을 구현해야 합니다."
                    )
                x = x.reshape(B, hw, hw, -1)
            else:
                # H, W가 지정되어 있다면 그대로 reshape
                x = x.reshape(B, height, width, -1)

        # (B, H, W, C) -> (B, C, H, W)로 순서 변환 후 첫 번째 Conv
...
class GTE(nn.Module):
    ...
    def forward(self, x):  # (B C H W)
        ...
        # In: [1, 384, 92, 305]
        x = self.conv(x)
        #-->  torch.Size([1, 192, 92, 305])

        y = x.permute(0, 2, 3, 1).contiguous()
        #-->  torch.Size([1, 92, 305, 192])

        y = self.mlp(y)
        #-->  torch.Size([1, 92, 305, 48])

        y = y.permute(0, 3, 1, 2).contiguous()
        #-->  torch.Size([1, 48, 92, 305])
        return y  # (B C H W)
...
class EDSABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        #self.mlp = Group_Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, norm_layer=norm_layer if self.use_mlp_norm else None, group_num=1)
        self.mlp = Group_Mlp_HW(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, norm_layer=norm_layer if self.use_mlp_norm else None, group_num=1,)# height=self.height, width=self.width)
        self.H = None
        self.W = None
    ...
    def forward(self, x, x_i, mask_matrix):
        H, W = self.H, self.W
        B, L, C = x.shape
        #print("real initial x shape", x.shape)
        ...
        shortcut = x.clone()
        #print("!!!!!!!!!!!",x.shape)#cw
        x = self.mlp(x + x_i, self.H, self.W)
        x = self.norm3(x)
        x = shortcut + self.drop_path(x)
        return x, x_i
...
class GET(nn.Module):
    ...
    def __init__(self,
        ...
        self.channel_embed = GTE(input_dim, hidden_dim, embed_dim, norm_layer=norm_layer, group_num=group_num, kernel_size=kernel_size)
...
class FeatureFusionBlock_L(nn.Module):
    """Feature fusion block.
    """
    ...
    def __init__(self, features):
        ...
        Args:
            features (int): number of features
        """
        super(FeatureFusionBlock_L, self).__init__()
    ...
    def forward(self, *xs):
        ...
        output = nn.functional.interpolate(
            output, size=(370, 1220), mode="bilinear", align_corners=True
        )
